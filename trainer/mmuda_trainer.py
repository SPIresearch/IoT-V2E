...
class MMUDA_trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            for index, train_batch in enumerate(self.src_loader):
                ...
                f1, f2, score = self.model(m1=sm1, m2=sm2)
                ft, pt = self.model(m1=tm1, m2=None)
                ...
                # if index % self.args.print_interval == 0:
    ...
    def val(self, epoch):
        ...
        # this_acc = self.metric["acc"](y_predict, y_true)
        # this_mf1 = self.metric["mf1"](y_predict, y_true)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for index, train_batch in enumerate(self.test_loader):
                ...
                y_predict = np.append(y_predict, pred)
                y_true = np.append(y_true, label)
                # acc = self.metric["acc"](pred, y)
                # accs.append(acc.item())
                # hd95 = self.metric["mf1"](pred, y)
                # hd95s.append(hd95)
        ...
        # dscs = torch.cat(dscs,dim=0)
        # #dscs = dist.all_gather(dscs,dist.ReduceOp)
        # hd95s = torch.cat(hd95s,dim=0)
        # dscs = torch.cat(dscs,dim=0)
