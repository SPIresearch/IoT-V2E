    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            for index, train_batch in enumerate(self.src_loader):
                ...
                fs,  ps = self.model(sm1, sm2)
                ft, pt = self.model(tm1, m2=None)
                # src_loss = self.loss_fn(ps, sy)
                src_loss = self.loss_fn(ps, sy, self.weight_class, self.device)
                da_loss = self.daloss(ps, fs, pt, ft) # p_s, f_s, p_t, f_t
                ...
                if index % self.args.print_interval == 0 and self.args.rank == 0:
                    Log.info("Epoch %d, Itrs %d/%d, Src_Loss%8f, Da_Loss%8f, Lr=%8f, Iter time=%f" %
