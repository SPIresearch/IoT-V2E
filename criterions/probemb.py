...
class MCSoftContrastiveLoss(nn.Module):
    ...
    def __init__(self, reduction='sum'):
        ...
        self.init_shift= 5   #hyper
        self.init_negative_scale= 5   #hyper
        shift = self.init_shift * torch.ones(1)
        negative_scale = self.init_negative_scale * torch.ones(1)
        ...
        self.num_samples = 7  #hyper

        self.uniform_lambda = 10  #hyper
        self.vib_beta =0.001   #hyper
    ...
    def _compute_loss(self, input1, input2):
        ...
        logits = -self.negative_scale.to(distance.device) * distance + self.shift.to(distance.device)   #公式3
