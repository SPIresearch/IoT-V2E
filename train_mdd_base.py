...
from trainer.mdd_trainer import MDD_trainer
...
def get_argparser():
    ...
    parser.add_argument("--name", type=str, default='base_mdd')
...
def main():
    ...
    Log.init(logfile_level="info",
             stdout_level="info",
             log_file="guorongxiao/EEG_Video_Fusion/log/base_mdd_prostate.log",
    ...
    classifier = nn.Conv3d(64, 1, 1)
    daloss = CMDD(classifier).to(args.device)
    daloss = DDP(daloss, device_ids=[args.rank], output_device=args.rank,broadcast_buffers=False,find_unused_parameters=True)
    ...
    trainer = MDD_trainer(model, daloss, optimizer, loss_fn, metric, src_loader, tgt_loader, test_loader, args, weights_for_each_class)
    trainer.train()
    trainer.test()
