...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, afr_reduced_cnn_size, dropout=0.1):
        ...
        # self.convs0 = CausalConv1d(afr_reduced_cnn_size, afr_reduced_cnn_size, kernel_size=7, stride=1)
        self.convs1 = CausalConv1d(afr_reduced_cnn_size, afr_reduced_cnn_size, kernel_size=7, stride=1)
        self.convs2 = CausalConv1d(afr_reduced_cnn_size, afr_reduced_cnn_size, kernel_size=7, stride=1)
    ...
    def forward(self, query, key, value):
        ...
        # query = self.convs0(query).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
        key = self.convs1(key).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
        value = self.convs2(value).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, afr_reduced_cnn_size, dropout):
        ...
        self.sublayer_output0 = SublayerOutput(size, dropout)
        self.sublayer_output1 = SublayerOutput(size, dropout)
        self.size = size
    ...
    def forward(self, x_in):
        ...
        x = self.sublayer_output0(query, lambda x_in: self.self_attn(query, x_in, x_in))  # Encoder self-attention
        out = self.sublayer_output1(x, self.feed_forward)
        return out
...
class AttnSleep(nn.Module):
    def __init__(self):
        ...
        attn1 = MultiHeadedAttention(h, d_model, afr_reduced_cnn_size)
        ff1 = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.tce1 = EncoderLayer(d_model, attn1, ff1, afr_reduced_cnn_size, dropout)

        attn2 = MultiHeadedAttention(h, d_model, afr_reduced_cnn_size)
        ff2 = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.tce2 = EncoderLayer(d_model, attn2, ff2, afr_reduced_cnn_size, dropout)

        self.norm = LayerNorm(self.tce2.size)
    ...
    def forward(self, x):
        x_feat = self.mrcnn(x)
        encoded_features = self.tce1(x_feat)  # encoded_features:tensor(128,30,100)
        encoded_features = self.tce2(encoded_features)
...
class XiaoTest_MRCNN(nn.Module):
    def __init__(self):
        super(XiaoTest_MRCNN, self).__init__()
        afr_reduced_cnn_size = 30

        self.mrcnn = MRCNN(afr_reduced_cnn_size)  # use MRCNN_SHHS for SHHS dataset
        self.fc1 = nn.Linear(30 * 100, 512, bias=True)

    def forward(self, x):
        out = self.mrcnn(x) # size(64,30,100)
        # print(out.size())
        out = out.contiguous().view(out.shape[0],-1)
        final_out = self.fc1(out)
        return final_out
...
class Xiao_Fusion_mm(nn.Module):
    ...
    def __init__(self, n_classes, video_input_channels, EEG_lastfeatures, Video_lastfeatures):
        ...
        self.attnsleep_features = AttnSleep()
        # self.attnsleep_features = XiaoTest_MRCNN()
